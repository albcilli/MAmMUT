...
from model_biked import TranClassNet, ResClassNet; 
...
fulldata = pd.read_csv('./BIKED_target.csv', index_col=0)
files = os.listdir('./images')
files = sorted(files, key=lambda x: int(x.split('.')[0]))
images = np.empty((len(files), 178, 384))
for i in range(len(files)):
    imagedata = cv2.imread(os.path.join('./images/', files[i]),0)
    imagedata = imagedata/255
    images[i, :, :] = imagedata

x = fulldata.iloc[:, :-1].values
...
for trainval_index, test_index in test_split.split(x, y):
    ...
    fulldata_trainval = fulldata.iloc[trainval_index, :]

    clf = RandomForestClassifier(n_estimators=100, max_depth=7, random_state=10)
    
    val_split = StratifiedShuffleSplit(n_splits=1, test_size=0.111, random_state=seed) #for nested-CV change with kfold
    for train_index, val_index in val_split.split(x_trainval, y_trainval):
        ...
        model = ResClassNet(input_size=train_dataset.x1.size(2)).to(device) #Tran
        
        #model_graph = draw_graph(TranClassNet(input_size=1), input_size=(1,1,178,384), expand_nested=True)
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_acc:
                ...
                torch.save(model.state_dict(), 'bk-best-model-parameters.pt')
                best_acc=val_acc
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=len(y_test), shuffle=False)


    final_model = ResClassNet(input_size=test_dataset.x1.size(2)).to(device)
    final_model.load_state_dict(torch.load('bk-best-model-parameters.pt'))
